...
lint:
	...
	$(GO_ENV) govulncheck ./...
	shellcheck -x install.sh uninstall.sh migrate-private-prealpha.sh packaging/debian/preinst packaging/debian/postinst packaging/debian/prerm packaging/debian/rootbroker-setup profiles/*/profile.sh profiles/grok/bin/grok-agent-launch profiles/grok/bin/grok-safe.in tests/*.sh
	actionlint
	$(MAKE) deadcode
