...
lint:
	...
	$(GO_ENV) govulncheck ./...
	shellcheck -x install.sh uninstall.sh migrate-private-prealpha.sh profiles/*/profile.sh profiles/grok/bin/grok-agent-launch profiles/grok/bin/grok-safe.in tests/*.sh
	actionlint
	$(MAKE) deadcode
