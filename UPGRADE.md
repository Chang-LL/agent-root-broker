...
Install state is root-owned at `/var/lib/hostctl/install-state`. Do not edit it. Changing the
...
`/etc/hostctl/config.json` before an upgrade because the current alpha installer renders its managed
configuration again.

If an install reports failure, first verify that `systemctl status hostctld` shows the restored
version as active. Do not manually repoint `/usr/local/libexec/hostctl-bin`; preserve the failure
