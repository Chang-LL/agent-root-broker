...
func Maintenance(args []string) int {
	if os.Geteuid() != 0 {
		fmt.Fprintln(os.Stderr, "hostctl-maint: root is required")
		return 1
	}
	...
	if len(args) != 2 || args[0] != "home-access" || (args[1] != "status" && args[1] != "revoke") {
		fmt.Fprintln(os.Stderr, "Usage: hostctl-maint [--config PATH] home-access status|revoke")
		return 2
	}
	cfg, err := config.Load(filepath.Clean(configPath))
	if err != nil {
		fmt.Fprintf(os.Stderr, "hostctl-maint: %v\n", err)
		return 1
	}
	if len(cfg.ApproverUsers) != 1 || len(cfg.AgentUsers) != 1 {
		fmt.Fprintln(os.Stderr, "hostctl-maint: home access requires exactly one approver and one agent")
		return 1
	}
