...
- `rootbroker version` and the release archive architecture;
...
- relevant `systemctl status rootbrokerd` or `journalctl -u rootbrokerd` output after redaction.

Never include access tokens, cookies, proxy credentials, private keys, real hostnames/IP addresses,
unredacted home paths, or sensitive command arguments. `rootbroker --json doctor` is safe to start
